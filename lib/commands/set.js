...
function* setRegistry(url, options) {
  if (!/^https?:\/\//.test(url)) throw new Error(`Invalid registry url ${url}, Must be a full url with 'http://'`);
  const originRegistry = yield getRegistry();
  yield spawn('npm', ['config', 'set', 'registry', url]);
  process.stdout.write(`Set npm registry: ${originRegistry.yellow} >>> ${url.green}\n`);
}
