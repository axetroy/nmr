...
program.command('set <registry>').alias('s').description('set registry').action(
  bootstrapWrapper(function(registry, options) {
    co(require('./commands/set')(registry, options)).catch(function(err) {
      console.error(err);
    });
  })
);

program.command('get').alias('g').description('get registry').action(
  bootstrapWrapper(function(options) {
